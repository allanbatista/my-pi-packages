...
8. Valid authorship + `execute` `pending|running|fail` retry-eligible → `read` `../batista-execute/SKILL.md`, apply inline. Every call uses the real interface (ver `../../references/PI_ADAPTATION.md`): `Agent({ subagent_type: "worker"|... })` — os agent files não pinam modelo/thinking (a chamada repassa `model`/`thinking` somente quando o usuário indicar explicitamente; omitidos, o child herda o modelo da sessão raiz); o child herda o cwd da sessão raiz — feature dir como `cwd`, contexto herdado ou valor efetivo divergente invalidates dispatch, promotes no state. Antes do primeiro dispatch real de um run/resume, rode o canary de runtime (cwd/modelo/conectividade + `max_turns` explícito; ver `../../references/WORKFLOW_COMMON.md`) e bloqueie com instrução de reiniciar o Pi no worktree correto quando o cwd divergir. Execution done → persist `Status: done` in `manifest.md` and `plan.md`. **Never end asking the user to run the phase.**
...
`Dir` é **sempre a feature dir canônica** `{project-root}/.features/{YYYY-MM-DD}_{HHMM}-{short-desc}/` — nunca um `./{nome}` novo. Sub-feature no mesmo worktree (sequencial/single) usa **a própria feature dir do epic**: os artefatos (`manifest.md`, `spec.md`, `ux.md`, `arch.md`, `plan.md`, `validation.md`) ficam juntos do `loop.md`, sem subpasta. Sub-feature paralela usa worktree próprio, cada um com a sua `.features/{...}/`. Criar `{feature-dir}/{nome}/` ou `{root}/{nome}/` é estrutura inválida: mover os artefatos para a feature dir canônica e corrigir referências antes de prosseguir.
